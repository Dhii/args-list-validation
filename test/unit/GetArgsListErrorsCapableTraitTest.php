...
class GetArgsListErrorsCapableTraitTest extends TestCase
{
    ...
    /**
     * Tests that `_getArgsListErrors()` fails as expected when a spec criterion is invalid.
     *
     * @since [*next-version*]
     */
    public function testGetArgsListErrorsFailureInvalidCriterion()
    {
        $args = [];
        $criterion = uniqid('criterion');
        $spec = [$criterion];
        $exception = $this->createOutOfRangeException('Invalid criterion');
        ...
        $_subject->_getArgsListErrors($args, $spec);
    }
}
